...
namespace TableStorage.Abstractions.POCO.Tests ;
    ...
    public class PocoTableStoreTests
    {
        ...
        public async Task insert_record_custom_json()
        {
            ...
            var ts = new TableStore<TableEntity>("TestEmployee",
                "UseDevelopmentStorage=true");

            var rawRecord = await ts.GetRecordAsync("42342", "42");
            var deptJson = rawRecord.GetString("DepartmentJson");
        ...
        public void insert_record_custom_property_converter()
        {
            ...
            var propertyConverters = new PropertyConverters<EmployeeWithHireDate>
            {
                [nameof(EmployeeWithHireDate.HireDate)] = new PropertyConverter<EmployeeWithHireDate>(
                    e => e.HireDate.ToString("yyyy-M-d"),
                    (e, p) => e.HireDate = DateTime.Parse(p.ToString())
                    )
            };
        ...
        {
            var record = tableStore.GetRecord("1", "1");
            Assert.AreNotEqual(default, record.Timestamp);
        }
        ...
        [TestMethod]
        public async Task get_record_with_no_results_async()
        {
            var record = await tableStore.GetRecordAsync("100", "10");
            Assert.IsNull(record);
        }
    ...
    [TestMethod]
        ...
        public void insert_record_with_fixed_partition_key()
        {
            ...
            var ts = new TableStore<TableEntity>("TestEmployee", "UseDevelopmentStorage=true",
            ...
            Assert.AreEqual("Mr. Jim CEO", record.GetString("Name"));
        }
        ...
        public void insert_record_with_fixed_partition_key_and_custom_property_converter()
        {
            ...
            var rKeyMapper = new KeyMapper<EmployeeWithHireDate, int>(e => e.Id.ToString(), int.Parse, e => e.Id,
                id => id.ToString());
            var keysConverter = new CalculatedKeysConverter<EmployeeWithHireDate, int, int>(pKeyMapper, rKeyMapper, propertyConverters);
        ...
        public void insert_multiple_record_with_fixed_partition_key()
        {
            ...
            Assert.IsTrue(records.Any(r => r.RowKey == "55" && r.GetString("Name") == "Mr. Ted QA"));
        }
        ...
        public void insert_record_with_reverse_sequential_row_key_sorts_correctly()
        {
            var pKeyMapper = new KeyMapper<Employee, int>(e => e.Id.ToString(), int.Parse, e => e.Id,
                id => id.ToString());
            var rKeyMapper = new SequentialKeyMapper<Employee, int>();
